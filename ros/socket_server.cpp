...
SocketServer::SocketServer(int port){
    ...
    sockaddr_in_server_.sin_addr.s_addr = INADDR_ANY;
...
bool SocketServer::open_socket(void){
    ...
    if(bind(socket_server_, (struct sockaddr *)&sockaddr_in_server_, sizeof(sockaddr_in_server_)) < 0){
        cout << "[ERROR]: [socket_server]: Open socket bind FAILURE" << endl;
        return false;
    }

    if(listen(socket_server_, 1) < 0){ //1クライアントまで接続可能
    ...
    socket_client_ = accept(socket_server_, (struct sockaddr *)&sockaddr_in_client_, (socklen_t *)&address_len);
    if(socket_client_ < 0){
...
bool SocketServer::close_socket(void){
    ...
    if(close(socket_client_) < 0 || close(socket_server_) < 0){
        cout << "[ERROR]: [socket_server]: Close socket close FAILURE" << endl;
        return false;
    }
